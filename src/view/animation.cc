...
void MainWindow::updateDisplayWidgets() {
    ...
    ui->tabWidgetDisplay->addTab(nullptr, "Maze");
    ui->tabWidgetDisplay->addTab(cave_painter, "Cave");
}

void MainWindow::switchTab(QAbstractButton *button) {
    int index = 0;
    if (ui->pushButtonCave == button) index = 1;
...
void MainWindow::on_pushButtonLoadMaze_clicked() {
    ...
    ui->lineEditMazeRows->setEnabled(!ui->lineEditMazeRows->isEnabled());
    ui->lineEditMazeCols->setEnabled(!ui->lineEditMazeCols->isEnabled());
}
